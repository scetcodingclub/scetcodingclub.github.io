...
if (contactForm) {
    contactForm.addEventListener('submit', function (e) {
        ...
        let body = `Hello NEXUS Team,%0D%0A` +
            `%0D%0AI would like to get in touch with you regarding: ${subject}%0D%0A` +
            `%0D%0A${message}%0D%0A` +
            `%0D%0ALooking forward to your response.%0D%0A`;

        // Add contact info and message
        body += `%0D%0AName: ${name}%0D%0A`;
